...
$MVCfactory = new MVCFactory();
	...
	//add new signup
	$app->post("/signup", function () use ($app , $MVCfactory)
	{		
...
//users  Api
	$app->get("/users/emailvalid/:email", function ($email) use ($app , $MVCfactory)
	{
	
			$parameters["id"]=$email;
			$parameters["id2"]="user_email";
			$MVCfactory->makeUsersMVC(ACTION_SEARCHFOR ,$app , $parameters);
	...
	$app->put("/users/:id/medicalcompleted", function ($id) use ($app , $MVCfactory)
	{
		//need to check these parameters
			$parameters["id"]=$id;
			$parameters["id2"]="user_completed_medical";
			$today = date("y.m.d");
			$parameters["id3"]=$today ;
			$MVCfactory->makeUsersMVC(ACTION_UPDATEVALUE ,$app , $parameters);
		
	});
	
	//ACTION_GETVALUE
	$app->get("/users/:id/medicalcompleted", function ($id) use ($app , $MVCfactory)
	{
		//need to check these parameters
			$parameters["id"]=$id;
			$parameters["id2"]="user_completed_medical";
			$MVCfactory->makeUsersMVC(ACTION_GETVALUE ,$app , $parameters);
...
//questions API
	...
	$app->get("/questions", function () use ($app , $MVCfactory)
	{
			$MVCfactory->makeQuestionMVC(ACTION_GETALL ,$app);
	});
	//get question with ID
	$app->get("/questions/:id", function ($id) use ($app , $MVCfactory)
	{
			$parameters["id"]=$id;
			$parameters["json"]=$app->request->getBody();
			$MVCfactory->makeQuestionMVC(ACTION_GET ,$app , $parameters);
	});
	//Add new question
	$app->post("/questions", function () use ($app , $MVCfactory)
	{
			$parameters["json"]=$app->request->getBody();
			$MVCfactory->makeQuestionMVC(ACTION_ADD ,$app , $parameters);
	});
	//Update a question
	$app->put("/questions/:id", function ($id) use ($app , $MVCfactory)
	{
			$parameters["id"]=$id;
			$parameters["json"]=$app->request->getBody();
			$MVCfactory->makeQuestionMVC(ACTION_UPDATE ,$app , $parameters);
	});
	//delete a question
	$app->delete("/question/:id", function ($id) use ($app , $MVCfactory)
	{
		$parameters["id"]=$id;
		$MVCfactory->makeQuestionMVC(ACTION_DELETE ,$app , $parameters);
	});	
