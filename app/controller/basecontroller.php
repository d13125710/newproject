...
abstract class baseController {
		...
		if($action!=null)
			{
				...
				switch($action) 
				{
						...
						case ACTION_UPDATEVALUE:
						$this->updateItemValue($parameteres);
						break;
						
						case ACTION_GETVALUE:
						$this->getItemValue($parameteres);
						break;
						
						case ACTION_SEARCHFOR:
						$this->getSearchForItem($parameteres);
						break;
	...
	*/
	...
	public function getItemValue($parameters)
	{
		
		$id1 = $parameters["id"];
		$id2  = $parameters["id2"];
		if($this->model->isExisting($id1)){
		
			$this->model->apiResponse=$this->model->getItemValue($id1 , $id2);
			$this->slimApp->response->setStatus(HTTPSTATUS_OK);
		}
		else 
		{
				//echo $this->model->isExisting($id1);
				//$this->model->apiResponse= $this->model->errordetails;
				$this->model->apiResponse = MESSAGE_INVALID_USER;
				$this->slimApp->response->setStatus(HTTPSTATUS_BADREQUEST);	
		}		
	}
	public function updateItemValue($parameters)
	{
		...
		$id3  = $parameters["id3"];
		if($this->model->isExisting($id1)){
			$this->model->apiResponse=$this->model->updateItemValue($id1 , $id2 , $id3);
			$this->slimApp->response->setStatus(HTTPSTATUS_OK);
		}
		else 
		{
	...
	public function getSearchForItem($parameters)
	{
		$id1 = $parameters["id"];
		$id2  = $parameters["id2"];
		
		$existing = $this->model->getSearchForItem($id1 , $id2);
		if(	$existing ==1 ) {
				$this->model->apiResponse=$existing;
				$this->slimApp->response->setStatus(HTTPSTATUS_OK);
		}
		else{
